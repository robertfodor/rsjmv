#' Diagnose Linear Regression Model
#'
#' Performs regression diagnostics including outlier/influence checks and residual analysis.
#'
#' @param model Fitted linear model object (e.g., from \code{lm()}).
#' @param data Data frame used to fit the model.
#' @param check_cook Logical; check Cook's distance.
#' @param check_dfbeta Logical; check DFBETAs.
#' @param check_leverage Logical; check leverage.
#' @param check_mahalanobis Logical; check Mahalanobis distance.
#' @param check_resid_plots Logical; create residual plots.
#' @param check_resid_sw_test Logical; perform Shapiro-Wilk test on residuals.
#' @param plot_diagnostics Logical; create diagnostic plots.
#' @param verbose Logical; print warnings.
#'
#' @return List with elements: results (diagnostic tables), plots (ggplot objects), formulas (cutoff formulas), cutoffs (numeric cutoff values).
#' @export
diagnose_regression <- function(model, data,
  ...
  if (check_resid_plots && plot_diagnostics && all(c(".pred_vals", ".raw_resid", ".std_resid") %in% names(diag_df))) {
    plots$resid_vs_pred <- ggplot2::ggplot(diag_df, aes(x = .pred_vals, y = .raw_resid)) +
      ggplot2::geom_point(shape = 21, size = 2.5, fill = "grey80", color = "black", alpha = 0.7) +
      ggplot2::geom_hline(yintercept = 0, color = "darkred", linewidth = 0.8) +
      ggplot2::scale_y_continuous(
        name = "Residuals",
        sec.axis = ggplot2::sec_axis(~ . / sd(., na.rm = TRUE), name = "Standardized Residuals")
      ) +
      ggplot2::labs(title = "Residuals vs. Predicted", x = "Predicted Values") +
      theme_rfapa()

    plots$hist_resid <- ggplot2::ggplot(diag_df, aes(x = .std_resid)) +
      ggplot2::geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "lightblue", color = "black", alpha = 0.7) +
      ggplot2::geom_density(color = "red", linewidth = 1) +
      ggplot2::labs(title = "Histogram of Standardized Residuals", x = "Standardized Residuals", y = "Density") +
      theme_rfapa()

    plots$qq_resid <- ggplot2::ggplot(diag_df, aes(sample = .std_resid)) +
      ggplot2::stat_qq(shape = 21, size = 2.5, fill = "grey80", color = "black", alpha = 0.7) +
      ggplot2::stat_qq_line(color = "red", linewidth = 1) +
      ggplot2::labs(title = "Q-Q Plot of Standardized Residuals", x = "Theoretical Quantiles", y = "Sample Quantiles") +
      theme_rfapa()
  }
  ...
  if (check_cook && ".cooks_d" %in% names(diag_df)) {
    ...
    if (plot_diagnostics) {
      plots$cooks_distance <- ggplot2::ggplot(diag_df, aes(x = .rownames, y = .cooks_d)) +
        ggplot2::geom_point(shape = 21, fill = "grey80", color = "black") +
        ggplot2::geom_segment(aes(xend = .rownames, yend = 0)) +
        ggplot2::geom_hline(yintercept = cutoff, color = "red", linetype = "dashed") +
  ...
  if (check_leverage && ".leverage" %in% names(diag_df)) {
    ...
    if (plot_diagnostics) {
      plots$leverage <- ggplot2::ggplot(diag_df, aes(x = .rownames, y = .leverage)) +
        ggplot2::geom_point(shape = 21, fill = "grey80", color = "black") +
        ggplot2::geom_hline(yintercept = cutoff, color = "red", linetype = "dashed") +
        ggplot2::labs(title = "Leverage", x = "Observation Index", y = "Leverage Value (Hat values)") +
        theme_rfapa()
    }
  }
  ...
  if (check_dfbeta) {
    ...
    if (plot_diagnostics) {
      ...
      plots$dfbeta <- ggplot2::ggplot(diag_df, aes(x = .rownames, y = .max_abs_dfbeta)) +
        ggplot2::geom_point(shape = 21, fill = "grey80", color = "black") +
        ggplot2::geom_segment(aes(xend = .rownames, yend = 0)) +
        ggplot2::geom_hline(yintercept = cutoff, color = "red", linetype = "dashed") +
        ggplot2::labs(title = "Maximum Absolute DFBETA per Case", x = "Observation Index", y = "Max(|DFBETA|)") +
        theme_rfapa()
    }
  }
      ...
      if (nrow(numeric_predictors_clean) > ncol(numeric_predictors_clean)) {
        ...
        if (any(variances < 1e-10)) {
          ...
          if (rcond(cov_matrix) > 1e-10) {
            ...
            diag_df <- dplyr::left_join(diag_df, maha_df, by = ".rownames")
            ...
            if (plot_diagnostics) {
              plots$mahalanobis <- ggplot2::ggplot(diag_df, aes(x = .rownames, y = .maha_dist)) +
                ggplot2::geom_point(shape = 21, fill = "grey80", color = "black") +
                ggplot2::geom_hline(yintercept = cutoff, color = "red", linetype = "dashed") +
                ggplot2::labs(title = "Mahalanobis Distance", x = "Observation Index", y = "Mahalanobis D-squared") +
                theme_rfapa()
            }
          } else {
